...
export function useLiveSocket() {
  ...
  useEffect(() => {
    ...
    function connect() {
      // Same-origin WebSocket through the Vite dev proxy (which forwards /ws to
      // the backend). Deriving the URL from location keeps laptop and compose
      // identical without a VITE_WS_URL override.
      const protocol = location.protocol === 'https:' ? 'wss:' : 'ws:'
      socket = USE_MOCKS
        ? new MockLiveSocket()
        : (new WebSocket(`${protocol}//${location.host}/ws/live`) as unknown as LiveSocketLike)
      ...
      socket.onmessage = (event) => {
        try {
          ...
          // The first frame is a HELLO fleet snapshot ({event:'HELLO',trucks}),
          // not a telemetry sample — and any other frame without a truckId is
          // likewise not one. Skip them rather than fabricating a sample.
          if (raw.event === 'HELLO' || !raw.truckId) return
